   ...
   function sectionlist(){
		...
		if($cs_type=='live'){
			if($livetype=='qcloudlive'){
				...
				foreach($sc_data as $n=> $val){
					$liveID=$sc_data[$n]['channelid'];
					$postdata = "bizCode=".$bizCode."&userName=".$username."&pReqVULC.liveName=".$name."&token=".$token.'&pReqVULL.liveID='.$liveID;
					$curl = curl_init(); 
					...
					$state=$data['contUL'];
					$sc_data[$n]['livestate']=$state['liveStatus'];
   ...
   function courseedit(){
	...
	function ziyuanku2(){
		...
		if($yuntype=='qcloud'){
			...
			foreach($video  as $n=> $val){
				$video[$n]['name']=$video[$n]['title'];								$video[$n]['videoid']=$video[$n]['videoID'];
				$video[$n]['filesize']=round($video[$n]['videoSize']/1024/1024,1);
				$video[$n]['ctime']=$video[$n]['duration'];access_url;
	...
	function addzhibo_post(){
		...
		if($livetype=='qcloudlive'){
			...
			$PRIVATE_PARAMS = array(
				'channelName'=>name,
				'outputSourceType'=> 2,
				'sourceList.1.name'=> 'video-1',
				'sourceList.1.type'=>1
			 );
			$Rsp=$this->CreateRequest($HttpUrl,$HttpMethod,$COMMON_PARAMS,$secretKey, $PRIVATE_PARAMS, $isHttps);
			$res=json_decode($Rsp,true);
			if($res.code==0){
				$this->live_obj = D("Common/live");
				$data['channelname']=$_SESSION['name'];
				$data['outputsourcetype']=3;
				$data['playerpassword']='';
				$data['upstream_address']=$res['channelInfo']['upstream_address'];
				$data['channelid']=$res['channel_id'];
				$data['appid']='';
				$data['sectionid']=$_SESSION['cs_id'];
				$check=$this->live_obj->where(array('sectionid'=>$_SESSION['cs_id']))->find();
				if($check){
					if($this->live_obj->where(array('sectionid'=>$_SESSION['cs_id']))->save($data)){
						unset($_SESSION['name']);
			...
			$isRecord=$_SESSION['isRecord'];
			$token=$this->gettoken();
			$bizCode='IPVULC';
			$url='http://open.wsview.com:8090/iWSViewPortalData?';
			$username=C('wsviewname');
			$postdata = "bizCode=".$bizCode."&userName=".$username."&pReqVULC.liveName=".$name."&token=".$token.'&pReqVULC.liveType=1'.'&pReqVULC.isRecord='.$isRecord;
			$curl = curl_init(); 
			curl_setopt($curl, CURLOPT_URL, $url); 
			curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);  
			curl_setopt($curl, CURLOPT_POST, 1); 
			...
			$data = json_decode(json_encode($xml),TRUE);
				if($data['resultCode']==0){
					...
					$data['outputsourcetype']=3;
					$data['playerpassword']='';
					$data['channelid']=$data['liveID'];
					$data['appid']='';
					$data['sectionid']=$_SESSION['cs_id'];
					$check=$this->live_obj->where(array('sectionid'=>$_SESSION['cs_id']))->find();
					if($check){
						$this->live_obj->where(array('sectionid'=>$_SESSION['cs_id']))->save($data);
						unset($_SESSION['name']);
						unset($_SESSION['cs_id']);
						unset($_SESSION['isRecord']);
						$this->success('创建直播间成功！');
