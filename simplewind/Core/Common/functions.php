...
function ToLog($level, $message){
	switch ($level)
	{
	case 1:  // 严重错误: 导致系统崩溃无法使用
	  Think\Log::record($message, Think\Log::EMERG);
	  break;  
	case 2:  // 警戒性错误: 必须被立即修改的错误
	  Think\Log::record($message, Think\Log::ALERT);
	  break;
	case 3:  // 临界值错误: 超过临界值的错误，例如一天24小时，而输入的是25小时这样
	  Think\Log::record($message, Think\Log::CRIT);
	  break;
	case 4:  // 一般错误: 一般性错误
	  Think\Log::record($message, Think\Log::ERR);
	  break;
	case 5:  // 警告性错误: 需要发出警告的错误
	  Think\Log::record($message, Think\Log::WARN);
	  break;
	case 6:  // 通知: 程序可以运行但是还不够完美的错误
	  Think\Log::record($message, Think\Log::NOTICE);
	  break;
	case 7:  // 信息: 程序输出信息
	  Think\Log::record($message, Think\Log::INFO);
	case 8:  // 调试: 调试信息
	  Think\Log::record($message, Think\Log::DEBUG);
	case 9:  // SQL：SQL语句 注意只在调试模式开启时有效
	  Think\Log::record($message, Think\Log::SQL);	  	  
	  break;	  	  	  	  	  
	default:
	  Think\Log::record($message, Think\Log::DEBUG);
	}
