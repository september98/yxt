...
class WxPayApi
{
	...
	public static function sendredpack($inputObj, $timeOut = 6){
		...
		$inputObj->SetMch_id(WxPayConfig::MCHID);//商户号		
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
		$inputObj->SetClient_ip(WxPayConfig::CLIENT_IP);
		$inputObj->SetSign();//签名
		
		
		$xml = $inputObj->ToXml();
		classLogger::getInstance()->Log(7,"sendredpack xml=" .$xml);
	 ...
	 * @return multitype:
	 * 
	 * $this->setParameter('partner_trade_no',$this->getBillNO());     // 商户订单号	partner_trade_no
	$this->setParameter('openid',$wecha_id);               // 用户openid	openid
	$this->setParameter('check_name','NO_CHECK');           // 校验用户姓名选项	check_name
	$this->setParameter('amount',$amount);               // 金额	amount
	$this->setParameter('desc','企业微红包' . $remark);                 // 企业付款描述信息	desc
	 */
	public static function transfer($inputObj, $timeOut = 6){
		$inputObj = new WxPayTransfer();
		$url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers";
		$inputObj->SetAppid(WxPayConfig::APPID);//公众账号ID
		$inputObj->SetMch_id(WxPayConfig::MCHID);//商户号
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
		$inputObj->SetSpbill_create_ip(WxPayConfig::CLIENT_IP);
		$inputObj->SetSign();//签名
	
	
		$xml = $inputObj->ToXml();
		classLogger::getInstance()->Log(7,"transfer xml=" .$xml);
